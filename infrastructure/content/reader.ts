...
import type { MateriaContent, TemaNode, SeccionNode, TemaData } from './models'
...
export async function getMateriaContent(materiaSlug: string): Promise<MateriaContent | null> {
  ...
  const entries = (fs.readdirSync(materiaPath, { withFileTypes: true }) as unknown as fs.Dirent[]).sort((a, b) =>
...
export async function getAllTemaParams() {
  ...
  const materias = (fs.readdirSync(CONTENT_DIR, { withFileTypes: true }) as unknown as fs.Dirent[])

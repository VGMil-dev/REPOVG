...
import { User, BookOpen, Shield, Clock, Filter, Search, X, Trash2, Zap, Loader2 } from "lucide-react";
...
interface AccesoListProps {
  initialAccesos: AccesoRow[];
  materias: { id: string; titulo: string }[];
  onRevocar: (id: string) => Promise<void>;
  onCambiarTipo: (id: string, nuevoTipo: "activo" | "historico") => Promise<void>;
}

export default function AccesoList({ 
  initialAccesos, 
  materias,
  onRevocar,
  onCambiarTipo
                ...
                <tr key={a.id} className="group/row hover:bg-brand-500/[0.03] transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                       <button
                        onClick={async () => {
                          setLocalLoading(a.id);
                          await onCambiarTipo(a.id, a.tipo === "activo" ? "historico" : "activo");
                          setLocalLoading(null);
                        }}
                        disabled={localLoading === a.id}
                        className="p-2 hover:bg-brand-500/10 rounded-lg text-brand-500/50 hover:text-brand-500 transition-colors"
                        title="Cambiar tipo de acceso"
                      >
                        {localLoading === a.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Zap className="w-3 h-3" />}
                      </button>
                      <button
                        onClick={async () => {
                          if (confirm("¿Seguro que deseas revocar este acceso?")) {
                            setLocalLoading(a.id);
                            await onRevocar(a.id);
                            setLocalLoading(null);
                          }
                        }}
                        disabled={localLoading === a.id}
                      ...
                      >
                        {localLoading === a.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Trash2 className="w-3 h-3" />}
                      </button>
                    </div>
                  </td>
                </tr>

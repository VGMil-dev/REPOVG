...
import { X, Key, User, BookOpen, Clock, ShieldCheck, Loader2, CheckCircle } from "lucide-react";

interface AsignarAccesoFormProps {
  open: boolean;
  ...
  onTipoChange: (tipo: "activo" | "historico") => void;
  usuarios: { id: string; nombre: string | null; email: string; rol?: string }[];
  materias: { id: string; titulo: string }[];
  semestres: { id: string; nombre: string }[];
}

export default function AsignarAccesoForm({ 
  open, 
  onOpenChange, 
  loading, 
  error, 
              ...
              <form onSubmit={onSubmit} className="space-y-6">
                ...
                {success && (
                  <div className="p-4 bg-brand-500/10 border border-brand-500/20 rounded-xl text-center">
                    <Typography variant="terminal-sm" className="!text-brand-500 uppercase flex items-center justify-center gap-2">
                      <CheckCircle className="w-4 h-4" /> ACCESO VINCULADO CORRECTAMENTE
                    </Typography>
                  </div>
                )}
